...
- **MaxListenersExceededWarning**: Node warns when more than 10 listeners are added to an EventEmitter (default `maxListeners` is 10). The main window and other created windows (login, viewer, Kick login, Destiny detached) can accumulate listeners (e.g. `closed`, webRequest, etc.). We call `setMaxListeners(20)` (or `40` for the main window) on each `BrowserWindow` when created to avoid the warning.
